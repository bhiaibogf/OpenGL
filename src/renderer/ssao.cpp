...
void SSAO::Generate() {
    ...
    for (unsigned int i = 0; i < 64; ++i) {
        ssao_shader_.setVec3("samples[" + std::to_string(i) + "]", ssao_kernel[i]);
    }
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, Sampler::GenerateNoises());
...
std::uniform_real_distribution<float> Sampler::random_float_ = std::uniform_real_distribution<float>(0.0, 1.0);
std::default_random_engine Sampler::generator_ = std::default_random_engine();

std::vector<glm::vec3> Sampler::kernel;

// generate sample kernel
std::vector<glm::vec3> Sampler::GenerateKernel() {
    if (kernel.empty()) {
        for (unsigned int i = 0; i < 64; ++i) {
            glm::vec3 sample(random_float_(generator_) * 2.0 - 1.0, random_float_(generator_) * 2.0 - 1.0,
                             random_float_(generator_));
...
unsigned int Sampler::GenerateNoises() {
    if (noise_map_ == 0) {
        std::vector<glm::vec3> noise_list;
        for (unsigned int i = 0; i < 16; i++) {
            // rotate around z-axis (in tangent space)
            glm::vec3 noise(random_float_(generator_) * 2.0 - 1.0, random_float_(generator_) * 2.0 - 1.0, 0.0f);
            noise_list.push_back(noise);
        }
        glGenTextures(1, &noise_map_);
        glBindTexture(GL_TEXTURE_2D, noise_map_);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, 4, 4, 0, GL_RGB, GL_FLOAT, &noise_list[0]);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
